...
TanStack Start (React) on Cloudflare Workers, Turso (libSQL) database, Cloudflare R2 + Image Transformations for photos, Tailwind v4 + shadcn/ui, Zustand for cart state. Type-safe end to end. Free at the documented scale.

See [`ARCHITECTURE.md`](./ARCHITECTURE.md) for the canonical reference.

## Local development

Local dev runs against a local libSQL server. Two terminals:
...
See [`OPERATIONS.md`](./OPERATIONS.md) for the full secret/rotation policy.
...
- [`OPERATIONS.md`](./OPERATIONS.md) - re-bootstrap and recovery notes: one-time Cloudflare setup, deploy verification, wrangler footguns, secret rotation.
- [`AGENTS.md`](./AGENTS.md) - implementation guardrails for coding agents or humans.
