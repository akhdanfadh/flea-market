...
- `/$slug/` detail page; nav-home happens via the global `SiteHeader`'s logo + wordmark Link (`search={() => ({})}` to clear filters). The page also has an explicit back link at the top - "Back to catalog" -> `/` by default, "Back to items" -> `/admin/` when arrived from the admin table (signalled via `?from=admin` on the inbound URL, validated by a Zod literal in `searchSchema`). Both targets are forward navigations to known routes rather than a `history.back()` intercept, which would walk visitors off-site if they arrived via an external link (`canGoBack()` alone can't tell the difference; compare to the modal's `modalPushedBySessionRef` guard, which doesn't translate to a route that mounts fresh per navigation). Trade-off: clicking back lands on the section root without previously-applied filter state, accepted to never walk visitors off-site. `throw notFound()` on unknown slugs.
...
- **Global chrome**: `SiteHeader` (logo + "Akhdan's Flea Market" wordmark on the left, `LanguagePill` on the right) and `SiteFooter` (centered copyright + a small icon button on the right) live in `__root.tsx` around `<Outlet />`. The footer icon is a `ShieldIcon` Link to `/admin/` on public pages and `/admin/login/`; it flips to a `LogOutIcon` POST-form to `/admin/logout/` whenever the visible pathname is under `/admin/` (excluding `/admin/login/`), since reaching such a path implies an authed session through the `_auth` guard. Pathname is read via `useLocation` and is a sufficient proxy for "show logout" without surfacing the HttpOnly cookie state to the client. Footer pins to the viewport bottom via `body.flex.min-h-dvh` + `main.flex-1`. Per-route `<title>` for `/$slug/` via the route's `head({ loaderData })` so SSR ships the correct title for shared links; the modal-over-list case on `/` (URL masked to `/$slug/`, matched route still `/`, so the route head doesn't fire) updates `document.title` client-side via a `useEffect` keyed on `activeModalRow`, restoring the prior title on close. Favicon set sourced from `akhdan.dev/static/favicon/` (svg + ico + png + apple-touch + webmanifest) under `public/favicon/`.
...
- Pathless `/admin/_auth` layout (`src/routes/admin/_auth.tsx`): `beforeLoad` calls `requireAdminSession` (server fn reading `getCookie(ADMIN_SESSION_COOKIE)` + `verifyCookie`); throws `redirect({ to: '/admin/login/' })` on missing/invalid. Layout component is intentionally chrome-less - just the page container, `<Outlet />`, and an admin-scoped `<Toaster>`. There is no in-layout "Admin" header or log-out button: the global `SiteFooter` shield icon flips to a `LogOutIcon` POST-form to `/admin/logout/` whenever the pathname is under `/admin/` (excluding `/admin/login/`), so logout lives in one consistent corner across every authed admin page. The pathless `_auth` shape guards everything under `_auth/` while leaving `login.tsx`, `logout.ts`, and `api/upload.ts` as un-guarded siblings.
